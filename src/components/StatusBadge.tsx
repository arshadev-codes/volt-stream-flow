...
const MAP: Record<ReactorPhase, { label: string; dot: string; text: string; bg: string; ring: string }> = {
  ...
  peak:      { label: "PEAK HOLD",  dot: "bg-orange-400",   text: "text-orange-200",   bg: "bg-orange-500/15",  ring: "ring-orange-400/40" },
...
export function StatusBadge({ status }: { status: ReactorPhase }) {
  const s = MAP[status];
  const animated = status === "ramp_up" || status === "peak" || status === "decay";
  return (
