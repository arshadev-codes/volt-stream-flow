...
export function VoltageCurrentGraph({
  ...
  useEffect(() => {
    ...
    const wasZoomed = prevMin != null && prevMax != null &&
      !(prevMin <= baseXMin + 1e-9 && prevMax >= baseXMax - 1e-9);
    u.setData(data, !wasZoomed);
    if (wasZoomed && prevMin != null && prevMax != null) {
      u.setScale("x", { min: prevMin, max: prevMax });
    } else {
      u.setScale("x", { min: baseXMin, max: baseXMax });
    }
